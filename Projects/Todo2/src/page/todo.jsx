  ...
  return (
    ...
    <div className='flex p-25 justify-center items-center flex-col '>
      <div className="">
        ...
        <button className='w-20 h-12.5 rounded-sm font-bold text-white bg-blue-600 ml-2' onClick={AddTask}>Add</button>
      </div>
      <div className="flex mt-8 gap-2.5 ">
        <input className="h-10 w-48 border-2 border-gray-300 rounded-sm pl-1.5" placeholder='Search' type="text" onChange={(e)=>setSearch(e.target.value)}/>
      </div>
      {
        filteredTasks.map(task => (
        <div className="flex text-center mt-10 justify-between w-xl border-2 border-gray-300 rounded-sm p-5">
